# WebDisplay 网页展示程序 2.3.0

基于 C#、.NET 10、WinUI 3 和 Microsoft Edge WebView2 的 Windows 桌面程序，用于在展示屏上持续打开指定网页。2.3.0 新增显示网页滚动条开关，并保留 2.2.0 加入的网页缩放。程序采用常规多文件发布并提供 Inno Setup 安装包，程序和运行依赖在安装时展开到固定目录，启动时直接加载。
...
推荐运行安装包 `dist\WebDisplay-Setup-2.3.0-x64.exe`。默认只为当前 Windows 用户安装到：
