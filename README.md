# WebDisplay 网页展示程序 2.1.0

基于 C#、.NET 10、WinUI 3 和 Microsoft Edge WebView2 的 Windows 桌面程序，用于在展示屏上持续打开指定网页。2.1.0 采用常规多文件发布并提供 Inno Setup 安装包，程序和运行依赖在安装时展开到固定目录，启动时直接加载。
...
推荐运行安装包 `dist\WebDisplay-Setup-2.1.0-x64.exe`。默认只为当前 Windows 用户安装到：
...
安装后启动 WebDisplay，填写展示网址并保存设置。首次运行时可以选择刷新、置顶、全屏、防止休眠和主题。
