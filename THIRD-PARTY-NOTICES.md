...
仅在确认可信的内部展示网站需要时开启该选项。忽略证书错误后无法可靠验证网站身份，可能遭到网站冒充或中间人攻击；该选项不保证解决 TLS 协议、加密算法协商或 mTLS 客户端证书认证问题。

Node.js 仅由开发者在显式运行包含 HTTPS 检查的 `--smoke-test` 时自行提供，未随发布包或安装包分发，不属于产品正常运行或 `--self-test` 的依赖。
