...
public sealed partial class MainWindow
{
    ...
    private const string SmokeHtml = "<!doctype html><html><meta charset='utf-8'><title>WebDisplay WinUI 3 test</title><style>:root{color-scheme:light dark}body{font:24px system-ui;padding:60px;min-width:1800px;min-height:1800px}h1{font-size:36px}::-webkit-scrollbar{width:18px;height:18px}</style><body><div id='scroll-probe' style='overflow:scroll;width:200px;height:100px;border:0;padding:0'><div style='width:600px;height:600px;background:linear-gradient(royalblue,lightblue)'>Scroll</div></div><h1>网页展示器 · WinUI 3</h1><p id='result'>WebView2 rendering works.</p><p>系统主题 · 自动恢复 · 持续展示</p><div id='zoom-target' style='width:100px;height:20px;background:royalblue'></div><iframe id='zoom-frame' style='width:100px;height:30px;border:0' srcdoc='<!doctype html><html><body>Frame</body></html>'></iframe></body></html>";
    ...
    private async Task RunSmokeTestAsync()
    {
        ...
        try
        {
            ...
            foreach (var (theme, index, name) in new[] { (ElementTheme.Dark, 2, "dark"), (ElementTheme.Light, 1, "light") })
            {
                ...
                ((ScrollViewer)root.FindName("DisplayScroll")).ChangeView(null, 0, null, true);
                await Task.Delay(900);
                await SavePreviewAsync(root, "settings-display-" + name + ".png");
                ((FrameworkElement)root.FindName("CertificateSettingsCard")).StartBringIntoView(new BringIntoViewOptions { AnimationDesired = false, VerticalAlignmentRatio = 0 });
                await Task.Delay(500);
                await SavePreviewAsync(root, "settings-certificates-" + name + ".png");
            ...
            await AssertScrollbarVisibilityAsync(false);
            if (_store.Load().ShowScrollbars) throw new InvalidOperationException("Cancelled scrollbar change was saved");
            _smokeChecks.Add("Cancelling scrollbar changes preserves the current page and saved setting");
    ...
    private async Task RunScrollbarSmokeChecksAsync()
    {
        await AssertScrollbarVisibilityAsync(true);
        _settings.ShowScrollbars = false;
        if (!await ApplyBrowserScrollbarsAsync()) throw new InvalidOperationException("Scrollbar control is unavailable");
        await RefreshAsync();
        await WaitForSmokePageAsync();
        await AssertScrollbarVisibilityAsync(false);
        var core = _browser!.CoreWebView2;
        bool pageBarsHidden = JsonSerializer.Deserialize<bool>(await core.ExecuteScriptAsync("innerWidth === document.documentElement.clientWidth && innerHeight === document.documentElement.clientHeight"));
        if (!pageBarsHidden) throw new InvalidOperationException("The main page scrollbars were not hidden");
        _smokeChecks.Add("Native horizontal and vertical scrollbars can be hidden on the page and scrollable panels");

        string wheelJson = await core.ExecuteScriptAsync("JSON.stringify((() => { const r=document.getElementById('scroll-probe').getBoundingClientRect(); return {type:'mouseWheel',x:r.left+20,y:r.top+20,deltaX:0,deltaY:160}; })())");
        await core.CallDevToolsProtocolMethodAsync("Input.dispatchMouseEvent", JsonSerializer.Deserialize<string>(wheelJson)!);
        await Task.Delay(600);
        bool scrolled = JsonSerializer.Deserialize<bool>(await core.ExecuteScriptAsync("document.getElementById('scroll-probe').scrollTop > 0 && getComputedStyle(document.getElementById('scroll-probe')).overflow === 'scroll'"));
        if (!scrolled) throw new InvalidOperationException("Hidden scrollbars prevented mouse-wheel scrolling");
        await core.ExecuteScriptAsync("document.getElementById('scroll-probe').scrollTo(50,60);window.scrollTo(70,80)");
        bool bothAxesScroll = JsonSerializer.Deserialize<bool>(await core.ExecuteScriptAsync("scrollX > 0 && scrollY > 0 && document.getElementById('scroll-probe').scrollLeft > 0"));
        if (!bothAxesScroll) throw new InvalidOperationException("Hidden scrollbars prevented programmatic scrolling");
        _smokeChecks.Add("Mouse-wheel and programmatic scrolling remain available with hidden scrollbars");
        await core.ExecuteScriptAsync("window.scrollTo(0,0)");
        _settings.ShowScrollbars = true;
        if (!await ApplyBrowserScrollbarsAsync()) throw new InvalidOperationException("Could not restore scrollbars");
        await RefreshAsync();
        await WaitForSmokePageAsync();
        await AssertScrollbarVisibilityAsync(true);
        _smokeChecks.Add("Showing scrollbars restores the website's original scrolling styles");
    }

    private async Task AssertScrollbarVisibilityAsync(bool visible)
    {
        // The fixture gives native scrollbars an explicit width, avoiding an
        // overlay-scrollbar OS preference making both states measure as zero.
        int[] gaps = JsonSerializer.Deserialize<int[]>(await _browser!.CoreWebView2.ExecuteScriptAsync("(() => { const p=document.getElementById('scroll-probe'); return [p.offsetWidth-p.clientWidth,p.offsetHeight-p.clientHeight]; })()"))!;
        if (visible ? gaps[0] <= 0 || gaps[1] <= 0 : gaps[0] != 0 || gaps[1] != 0)
            throw new InvalidOperationException($"Scrollbar visibility {visible} has unexpected native gaps: {gaps[0]}, {gaps[1]}");
    }
    ...
    private async Task RunZoomSmokeChecksAsync()
    {
        ...
        var root = (FrameworkElement)dialog.Content;
        var scrollbarToggle = (ToggleSwitch)root.FindName("ShowScrollbarsToggle");
        if (!scrollbarToggle.IsOn) throw new InvalidOperationException("Scrollbar setting did not load current value");
        scrollbarToggle.IsOn = false;
        await AssertScrollbarVisibilityAsync(true);
        ...
        if (_settings.ShowScrollbars) throw new InvalidOperationException("Scrollbar preference did not persist");
        await ApplyBrowserZoomAsync();
        if (!await ApplyBrowserScrollbarsAsync()) throw new InvalidOperationException("Saved scrollbar preference failed to apply");
        await RefreshAsync();
        await WaitForSmokePageAsync();
        await AssertScrollbarVisibilityAsync(false);
        _smokeChecks.Add("Scrollbar choice persists and only applies after saving");
        await AssertZoomWidthAsync(137);
