...
internal static class AdminPipe
{
    ...
    internal static string ExecutablePath => Environment.ProcessPath
        ?? throw new InvalidOperationException("无法确定程序文件路径，请从已发布的 EXE 启动程序。");
    ...
    internal static async Task<string> SendAsync(AdminRequest request)
    {
        ...
        try
        {
            ...
            if (child is null)
                throw new InvalidOperationException("无法启动管理员配置程序。");
        ...
        {
            throw new InvalidOperationException("已取消管理员授权，系统设置未更改。", ex);
        }
        catch (OperationCanceledException ex)
        {
            throw new InvalidOperationException("管理员配置等待超时。请重新打开设置检查系统状态后重试。", ex);
        }
        finally
        {
        ...
        try
        {
            ...
            try
            {
                if (request.Version != 1 || (request.Operation != "restart" && request.Operation != "autologon"))
                    throw new InvalidOperationException("不支持的管理员操作。");
            ...
            catch (Exception ex)
            {
                ...
                response = new AdminResponse { Success = false, Message = "系统配置失败：" + ex.Message };
            }
            finally
            {
                request.Password = string.Empty;
    ...
    {
        using WindowsIdentity identity = WindowsIdentity.GetCurrent();
        string sid = identity.User?.Value ?? throw new InvalidOperationException("无法获取当前 Windows 用户标识。");
    ...
    private static async Task WriteAsync<T>(Stream pipe, T value, CancellationToken token)
    {
        ...
        try
        {
            if (body.Length > MaxMessageBytes)
                throw new InvalidOperationException("配置数据过长。");
    ...
    private static async Task<T> ReadAsync<T>(Stream pipe, CancellationToken token)
    {
        ...
        if (length <= 0 || length > MaxMessageBytes)
            throw new InvalidOperationException("无效的管理员通信数据。");
        byte[] body = new byte[length];
        try
        {
            await pipe.ReadExactlyAsync(body, token).ConfigureAwait(false);
            return JsonSerializer.Deserialize<T>(body) ?? throw new InvalidOperationException("管理员通信数据为空。");
        }
        finally
        {
            CryptographicOperations.ZeroMemory(body);
