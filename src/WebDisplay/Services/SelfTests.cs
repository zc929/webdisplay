...
internal static class SelfTests
{
    public static int Run(string directory)
    {
        ...
        Check("Settings persistence and corrupt-file recovery", () =>
        {
            ...
            var expected = new AppSettings { Url = "https://example.com/test", AutoRefreshEnabled = true, RefreshIntervalMinutes = 17, FullScreen = false, PreventSleep = false };
            store.Save(expected);
            var actual = store.Load();
            Assert(actual.Url == expected.Url && actual.RefreshIntervalMinutes == 17 && !actual.PreventSleep);
