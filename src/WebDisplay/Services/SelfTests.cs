...
internal static class SelfTests
{
    public static int Run(string directory)
    {
        ...
        {
            Assert(!new AppSettings().IgnoreCertificateErrors);
            var legacy = JsonSerializer.Deserialize<AppSettings>("{\"Url\":\"https://example.com\",\"ZoomPercent\":137,\"ShowScrollbars\":false}")!;
            Assert(!legacy.IgnoreCertificateErrors && legacy.ZoomPercent == 137 && !legacy.ShowScrollbars);
            legacy.IgnoreCertificateErrors = true;
            var enabled = JsonSerializer.Deserialize<AppSettings>(JsonSerializer.Serialize(legacy))!;
            var disabled = enabled.Clone();
            disabled.IgnoreCertificateErrors = false;
            Assert(enabled.IgnoreCertificateErrors && !disabled.IgnoreCertificateErrors);
        });
        ...
        Check("Web zoom accepts boundary and custom percentages, rejects out-of-range values", () =>
        {
            foreach (int zoom in new[] { 25, 80, 100, 125, 137, 500 }) new AppSettings { ZoomPercent = zoom }.Validate();
            foreach (int zoom in new[] { -100, 0, 24, 501, int.MaxValue }) Reject(() => new AppSettings { ZoomPercent = zoom }.Validate());
        });
        Check("Existing configurations keep their settings and default to 100 percent zoom", () =>
        {
            var legacy = JsonSerializer.Deserialize<AppSettings>("{\"Url\":\"https://example.com/board\",\"FullScreen\":false,\"ThemePreference\":\"Dark\",\"RefreshIntervalMinutes\":17}")!;
            legacy.Validate();
            Assert(legacy.ZoomPercent == 100 && legacy.Url.EndsWith("/board") && !legacy.FullScreen && legacy.ThemePreference == "Dark" && legacy.RefreshIntervalMinutes == 17);
        });
        Check("Legacy configurations keep scrollbars visible; hidden preference survives serialization", () =>
        {
            var legacy = JsonSerializer.Deserialize<AppSettings>("{\"Url\":\"https://example.com/board\",\"ZoomPercent\":137}")!;
            Assert(legacy.ShowScrollbars && legacy.ZoomPercent == 137);
            legacy.ShowScrollbars = false;
            var hidden = JsonSerializer.Deserialize<AppSettings>(JsonSerializer.Serialize(legacy))!;
            Assert(!hidden.ShowScrollbars && hidden.ZoomPercent == 137);
        });
